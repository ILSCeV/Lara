...
$(() => {
  ...
  $(".fa-sort, .fa-sort-up, .fa-sort-down").on({
    click: (e) => {
      sortLeaderboards($(e.target));
    }
  });

  $('#memberStatisticsTabs').find('thead').find('td').on('click', function () {
    sortLeaderboards($($(this).find('i').first()));
  });

  $(".statisticClubPicker").find("a").on('click', function () {
    const clubName = $(this).text().trim();
    localStorage.setItem('preferredStatistics', clubName);
  });

  $(".leaderboardsClubPicker").find("a").on('click', function () {
    const leaderBoardName = $(this).text().trim();
  ...
  if (!preferredStatistics) {
    ...
    preferredStatistics = userSection;
  }
...
$('[name^=show-stats-person]').on('click', function (e) {
  e.preventDefault();
    ...
    dialog.init(function () {
      ...
      for (let i = 0; i < response.length; i++) {
        $("#person-shifts-overview-body").append(
          ...
          + "<tr" + (!response[i]["isOwnClub"] ? " class=\"other-section text-body-secondary\"" : "") + ">"
          + "<td>" + (1 + i) + "</td>"
