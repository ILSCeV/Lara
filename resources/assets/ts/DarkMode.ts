...
(() => {
    ...
    const showActiveTheme = (theme : string, focus = false) => {
      ...
      const activeThemeIcon : HTMLElement = document.querySelector('i.theme-icon-active')
      const btnToActive : HTMLElement = document.querySelector(`[data-bs-theme-value="${theme}"]`)
      const iconOfActiveBtn : string = btnToActive.querySelector('i').getAttribute('href')
      ...
      activeThemeIcon.setAttribute('href', iconOfActiveBtn)
    ...
    window.matchMedia('(prefers-color-scheme: dark)').addEventListener('change', () => {
      const storedTheme : string = getStoredTheme()
