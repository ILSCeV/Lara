<div name="question">
    <div class="card col-md-8 col-sm-12 col-12">
        <div class="card-body">
            <div class="col-md-11 col-sm-11 col-10 ps-0">
                <h4 class="heading-reference">{{ __('mainLang.question') }} #</h4>
            </div>
            ...
            <div class="col-md-6 col-sm-6 col-12 ps-0">
                <fieldset>
                    <label name="quest_label">{{ __('mainLang.question') }}: &nbsp</label>
            ...
            <div class="col-md-6 col-sm-6 col-12 ps-4">
                <fieldset>
                    <select class="form-select" name="type_select[{{$id}}]" onChange="javascript:updateQuestionDisplay(this)">
                        <option value="1" data-icon="fa fa-file-text-o"
                                @if($question->field_type === 1) selected @endif >{{ __('mainLang.freeText') }}</option>
                        <option value="2" data-icon="fa fa-check-square-o"
                                @if($question->field_type === 2) selected @endif >{{ __('mainLang.checkbox') }}</option>
                        <option value="3" data-icon="fa fa-caret-square-o-down"
                                @if($question->field_type === 3) selected @endif >{{ __('mainLang.dropdown') }}</option>
                    </select>
                    <input class="hidden" type="hidden" name="hiddenField" value="nothingYet">
                </fieldset>
            </div>

            <div class="col-md-6 col-sm-6 col-12 ps-2">
                <fieldset class="checkbox entrylist">
                    ...
                    <label><input type="checkbox" name="required[{{$id}}]" @if($question->is_required) checked
                                  ...
                                  class="input_checkboxitem"> {{ __('mainLang.required') }}</label>
                </fieldset>
            </div>


            <div class="col-md-6 col-sm-6 col-12 ps-0" name="answerOptionsDiv">
                <div class="answ_option">
                    @include('partials.survey.edit.answerOptions')
                    <input class="btn btn-success btn-sm" name="answerButton"
                           value="{{ __('mainLang.addAnswerOption') }}"
