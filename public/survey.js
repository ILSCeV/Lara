...
// CONCATENATED MODULE: ./resources/assets/ts/lang/de.ts
...
// CONCATENATED MODULE: ./resources/assets/ts/lang/en.ts
...
// CONCATENATED MODULE: ./resources/assets/ts/lang/pirate.ts
/* harmony default export */ var pirate = ({
    "shiftTime": "Deckzeit",
    "onlyEmpty": "Nur FREIe Dienste",
...
// CONCATENATED MODULE: ./resources/assets/ts/Translate.ts
...
"use strict";
/* WEBPACK VAR INJECTION */(function($) {/* harmony import */ var _Translate__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(1);
/* harmony import */ var bootbox__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(2);
...
$("#button-create-survey").click(function () {
    ...
    if ($("#description").val().length > 1500) {
        errors.push(Object(_Translate__WEBPACK_IMPORTED_MODULE_0__[/* translate */ "a"])("descriptionTooLong"));
    }
    if (errors.length > 0) {
        bootbox__WEBPACK_IMPORTED_MODULE_1__["alert"](errors.map(function (err) { return "<p>" + err + "</p>"; }).join("\n"));
        return false;
    }
});
...
// EXTERNAL MODULE: ./resources/assets/ts/surveys/CreateSurvey.ts
var CreateSurvey = __webpack_require__(17);

// EXTERNAL MODULE: ./node_modules/jquery/dist/jquery.js
var jquery = __webpack_require__(0);

// CONCATENATED MODULE: ./resources/assets/ts/surveys/surveyEdit-Create-scripts.ts

var questionIdCounter = 0;
// Initialize State for questions and answers
jquery(function () {
    jquery('.selectpicker').each(updateQuestionDisplay);
    ...
    jquery("form").submit(function () {
        if (jquery('#btnAdd') !== '.click') {
            jquery(window).unbind('beforeunload');
        }
    });
    // Checks if user has made input -> if yes, a popup appears if user wants to leave the site
    jquery('.questions').add('.input_checkboxitem').add('.selectpicker').change(function () {
        jquery(window).bind('beforeunload', function () {
            return 'Beim Verlassen der Seite gehen alle Eingaben verloren.';
        });
    });
...
function setDeleteQuestionButtonStatus() {
    var amountOfQuestions = jquery('.questions').children().length;
    jquery('.btnRemoveQuestion').prop('disabled', amountOfQuestions === 1);
}
...
jquery('.btnRemoveQuestion').click(function () {
    jquery(this).closest('[name=question]').remove();
    setQuestionNumberDisplayed();
    setDeleteQuestionButtonStatus();
});
function updateQuestionDisplay(select) {
    var $typeSelect = jquery(select);
...
function updateAnswerOptionsIndices(question) {
    jquery(question).find('.answ_option').find('.input-group').find('input').each(function (index, input) {
        var oldName = jquery(input).prop('name');
        if (oldName) {
            ...
            jquery(input).prop('name', newName);
        }
    });
}
function addNewAnswerOption(button) {
    var $clone = jquery(button).prev().clone(true, true).insertBefore(button);
    $clone.find('input').val('');
    updateAnswerOptionsIndices(jquery(button).closest('[name^=question]'));
}
// Deleting answer options
function removeAnswerOption(answer) {
    var question = jquery(answer).closest('[name^=question]');
    if (jquery(question).find('.answ_option').children('div').length > 1) {
        jquery(answer).closest('.input-group').remove();
        updateAnswerOptionsIndices(question);
    }
}
function updateQuestionNumber(index, topDiv) {
    var $heading = jquery(topDiv).find('.heading-reference');
    var currentText = $heading.text();
    $heading.text(currentText.replace(/#.*/, '#' + (index + 1)));
}
function setQuestionNumberDisplayed() {
    jquery('div').filter('.questions').children().each(updateQuestionNumber);
}
function getLastQuestionId() {
    try {
        return parseInt(/questionText\[([0-9]*)\]/g.exec(jquery('.questions').children()
            .last()
...
var bootbox = __webpack_require__(2);

// CONCATENATED MODULE: ./resources/assets/ts/surveys/surveyView-scripts.ts
...
function getPasswordField() {
    return jquery('.card.bg-warning').find("[name^=password]");
}
...
jquery(document).ready(function () {
    jquery('.store').on('submit', function () {
...
jquery('.editButton').click(function () {
    ...
    jquery(this).addClass("editButton btn btn-success fa-floppy-o");
    jquery(this).removeClass("btn-primary fa-pencil");
    jquery(this).val('');
});
function change_to_submit(number) {
    if (jquery('#editButton' + number).attr('type') == 'button') {
        setTimeout(function () {
            jquery('#editButton' + number).attr('type', 'submit');
        }, 100);
    }
    else {
        setTimeout(function () {
            jquery('#editButton' + number).attr('type', 'button');
        }, 100);
    }
}
////////////////////////////////////////
...
jquery('input').focusout(function () {
    ...
    if (jquery(this).prop('id') === 'newName') {
        setTimeout(function () {
            jquery('#dropdown-menu_name').hide();
        }, 200);
    }
});
///////////////////////////////////
...
jquery(document).ready(function () {
    window.onresize = function () {
        var height_evaluation = jquery('#evaluation').height();
        jquery(document).find('.evaluation_heading').attr('style', 'height:' + height_evaluation + 'px;');
    };
});
/////////////////////
// EDITING ANSWERS //
...
function get_answer_row(number) {
    jquery('#get_row').attr('value', number);
}
jQuery(document).ready(function ($) {
...
// CONCATENATED MODULE: ./resources/assets/ts/surveys/surveyEntry.ts
